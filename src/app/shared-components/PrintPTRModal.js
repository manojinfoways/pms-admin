...
const PrintPTRModal = ({ openModal, setOpenModal, children, ...props }) => {
  ...
  useEffect(() => {
    if (props.ptrId) {
      ...
      axios
        ...
        .then((response) => {
          const requestedPTRData = response.data.result.find(
            (i) => i.id === props.ptrId
          );
          if (requestedPTRData) setCurrentPTRDetail(requestedPTRData);

          axios
            .post("/associate_taxinfo_list", {
              ptr_id: props.ptrId,
            })
            .then((res) => {
              if (res.status) {
  ...
  useEffect(async () => {
    if (
      (!_.isEmpty(ptrDetails) || !_.isEmpty(ptrAlphaRecords)) &&
      ptrDetails[0].id === props.ptrId
    ) {
      const documents = [].concat.apply(
        [],
        ptrDetails.map((i) => i.properties[0].docs)
      );
      const data = documents.map((row) => [
        ...
        row.priority_no,
        row.recorded_date
          ? moment(row.recorded_date).format("DD/MM/YYYY")
          : "-",
        row.associated_ct,
        row.sourcetype,
      ]);
  ...
  const handleInputChange = (e, index) => {
    if (e.target.checked) {
      setChecked((prev) => [
        ...prev,
        { name: e.target.value, label: e.target.value, id: index },
      ]);
    } else {
  ...
  const handleInfoInputChange = (e, index) => {
    if (e.target.checked) {
      ...
      setInfoChecked((prev) =>
        prev.filter((col) => col.name !== e.target.value)
      );
    }
  };
  ...
  const customFilter = () => {
    const docs = mixDocuments
      .filter((doc) => checked.map((i) => i.label).includes(doc[2]))
      .map((row) => {
    ...
    setCOT(
      docs
        .filter((i) => i.includes("O-C") || i.includes("A-C"))
        .sort((a, b) => a[6] - b[6])
    );
    setLegacy(
      sortDataByDate(docs.filter((i) => i.includes("O-L") || i.includes("A-L")))
    );
    setException(
      sortDataByDate(docs.filter((i) => i.includes("O-E") || i.includes("A-E")))
    );
    setNameCheck(
      sortDataByDate(docs.filter((i) => i.includes("O-N") || i.includes("A-N")))
    );
  ...
  const sortDataByDate = (data) => {
    return data.sort((obj1, obj2) => {
      let d1 =
        obj1[7] !== "-"
          ? moment(obj1[7], "DD/MM/YYYY").valueOf()
          : Infinity * -1;
      let d2 =
        obj2[7] !== "-"
          ? moment(obj2[7], "DD/MM/YYYY").valueOf()
          : Infinity * -1;
      return d2 - d1;
    });
  };
  ...
  const fetchData = async (id) => {
    setIsLoading(true);
    if (user.role.includes("admin")) await getPtrDetailsByIdAdmin(id);
                    ...
                    <tbody>
                      <tr>
                        <td className={classes.label}>
                          Requester:{" "}
                          <span>{currentPTRDetail.customer_name || "-"}</span>
                        </td>
                        <td className={classes.label}>
                          Request Type:{" "}
                          <span> {currentPTRDetail.request_type || "-"}</span>
                        </td>
                      </tr>
                      <tr>
                        <td className={classes.label}>
                          Property Type:{" "}
                          <span>{currentPTRDetail.property_type || "-"} </span>
                        </td>
                        <td className={classes.label}>
                          PTR: <span>{currentPTRDetail.id || "-"}</span>
                        </td>
                      </tr>
                      <tr>
                        <td className={classes.label}>
                        ...
                        <td className={classes.label}>
                          Dated:{" "}
                          <span>
                            {currentPTRDetail?.summary?.dated
                              ? new Date(
                                  currentPTRDetail.summary?.dated
                                ).toLocaleDateString()
                              : "-"}
                  ...
                  <table cellPadding={4} cellSpacing={4} border={0}>
                    <tbody>
                      <tr>
                        <td className={classes.label}>
                          Researcher:{" "}
                          <span>{currentPTRDetail.staff_username || "-"}</span>
                        </td>
                        <td className={classes.label}>
                          Reviewed By:{" "}
                          <span>
                            {currentPTRDetail.summary?.reviewed_by || "-"}
                          </span>
                        </td>
                      </tr>
                      <tr>
                        <td className={classes.label}>
                          Recorded Date:{" "}
                          <span>
                            {currentPTRDetail.summary?.recorded
                              ? new Date(
                                  currentPTRDetail.summary?.recorded
                                ).toLocaleDateString()
                              : "-"}
                        ...
                        <td className={classes.label}>
                          Updated Date:{" "}
                          <span>
                            {currentPTRDetail.summary?.updated_at
                              ? new Date(
                                  currentPTRDetail.summary?.updated_at
                                ).toLocaleDateString()
                              : "-"}
                      ...
                      <tr>
                        <td className={classes.label}>
                          Status:{" "}
                          <span>{currentPTRDetail.ptr_status || "-"}</span>
                        </td>
                        <td className={classes.label}>
                          Completion Date:{" "}
                          <span>
                            {currentPTRDetail.summary?.date_completed || "-"}
                          </span>
                        </td>
                      </tr>
                    </tbody>
                  ...
                  <table cellPadding={4} cellSpacing={4} border={0}>
                    <tbody>
                      <tr>
                        <td className={classes.label}>
                          Description:{" "}
                          <span>{currentPTRDetail.notes || "-"}</span>
                        </td>
                      </tr>
                    </tbody>
                  </table>
                  ...
                  <table cellPadding={4} cellSpacing={4} border={0}>
                    <tbody>
                      <tr>
                        <td className={classes.label}>
                          Transaction ID:{" "}
                          <span>{currentPTRDetail.transaction_id || "-"}</span>
                        </td>
                        <td className={classes.label}>
                          Owner(s) on Record:{" "}
                          <span>{currentPTRDetail.summary?.owners || "-"}</span>
                        </td>
                      </tr>
                    </tbody>
                  </table>
                  ...
                  <table cellPadding={4} cellSpacing={4} border={0}>
                    <tbody>
                      <tr>
                        <td className={classes.label}>
                          Tract:{" "}
                          <span>{currentPTRDetail.tract_std || "-"}</span>
                        </td>
                        <td className={classes.label}>
                          Block:{" "}
                          <span>{currentPTRDetail.block_std || "-"}</span>
                        </td>
                      </tr>
                    </tbody>
                  </table>
                  <table cellPadding={4} cellSpacing={4} border={0}>
                    <tbody>
                      <tr className={classes.areaWrapper}>
                        <td className={classes.label}>
                          Lot: <span>{currentPTRDetail.lot_std || "-"}</span>
                        </td>
                        <td className={classes.label}>
                          Unit: <span>{currentPTRDetail.unit_std || "-"}</span>
                        </td>
                        <td className={classes.label}>
                          Phase: <span>{currentPTRDetail.phase || "-"}</span>
                        </td>
                      </tr>
                    </tbody>
                  </table>
                    ...
                    <tbody>
                      <tr>
                        <td className={classes.label}>
                          Drawing Number:{" "}
                          <span>
                            {currentPTRDetail.summary?.drawing_number || "-"}
                          </span>
                        </td>
                        <td className={classes.label}>
                          LM Check:{" "}
                          <span>
                            {currentPTRDetail.summary?.lmcheck || "-"}
                          </span>
                        </td>
                        <td className={classes.label}>
                          Area:{" "}
                          <span>{currentPTRDetail.summary?.area || "-"}</span>
                        </td>
                      </tr>
                      <tr>
                        <td className={classes.label}>
                          Territory of Guam:{" "}
                          <span>
                            {currentPTRDetail.summary?.territoryofguam || "-"}
                          </span>
                        </td>
                        <td className={classes.label}>
                          Description on Map:{" "}
                          <span>
                            {currentPTRDetail.summary?.desc_on_map || "-"}
                          </span>
                        </td>
                        <td className={classes.label}>
                          Basic Lot:{" "}
                          <span>
                            {currentPTRDetail.summary?.basic_lot || "-"}
                          </span>
                        </td>
                      </tr>
                      <tr>
                        <td className={classes.label}>
                          Owners CT:{" "}
                          <span>
                            {currentPTRDetail.summary?.ownersCT || "-"}
                          </span>
                        </td>
                        <td className={classes.label}>
                          Last CT:{" "}
                          <span>{currentPTRDetail.summary?.lastCT || "-"}</span>
                        </td>
                        <td className={classes.label}>
                          Estate Number:{" "}
                          <span>
                            {currentPTRDetail.summary?.estate_no || "-"}
                          </span>
                        </td>
                      </tr>
                    </tbody>
      ...
      <Grid container spacing={2} className={classes.headerWrapper}>
        ...
        <Grid item xs={9} className="p-36">
          <div className="d-flex justify-end items-center my-auto">
            <h4 className={classes.printTitle}>PMS</h4>
          </div>
        </Grid>
        ...
        <Box className={clsx(props.className, classes.container, "rounded-12")}>
          ...
          {pages.map((i, index) => [
            <FormControlLabel
              ...
              control={
                <Checkbox
                  checked={
                    checked.filter((col) => col.label === i.label).length
                  }
                  value={i.label}
          ...
          {infoPage.map((i, index) => [
            <FormControlLabel
              ...
              control={
                <Checkbox
                  checked={
                    infoChecked.filter((col) => col.name === i.name).length
                  }
                  value={i.name}
          ...
          {summaryPage.map((i, index) => [
            <FormControlLabel
              ...
              control={
                <Checkbox
                  checked={
                    summaryChecked.filter((col) => col.name === i.name).length
                  }
                  value={i.name}
            ...
            PTRNotes.map((i, index) => [
              <FormControlLabel
                ...
                control={
                  <Checkbox
                    checked={
                      notesChecked.filter((col) => col.name === i.name).length
                    }
                    value={i.name}
          ...
          <Print
            ...
            subTitle={
              props.subTitle +
              "- " +
              (currentPTRDetail.property_descriptor_std ||
                currentPTRDetail.property_descriptor)
            }
          ...
          >
            ...
            {checked.length > 0 && (
              <>
                ...
                {checked.filter((i) => i.label === "CHAIN_TITLE").length ===
                  1 && (
                  <>
                ...
                {checked.filter((i) => i.label === "LEGACY_PTR").length ===
                  1 && (
                  <>
                ...
                {checked.filter((i) => i.label === "NAME_CHECK").length ===
                  1 && (
                  <>
                ...
                {checked.filter((i) => i.label === "EXCEPTION").length ===
                  1 && (
                  <>
            ...
            {notesChecked.length > 0 && !user.role.includes("user") && (
              <>
                ...
                <Box className="border-none mx-80 p-30">
                  <div
                    className={clsx("justify-center text-lg", classes.notes)}
                    dangerouslySetInnerHTML={{
                      __html: currentPTRDetail.admin_notes,
                    }}
                  />
                </Box>
              </>
