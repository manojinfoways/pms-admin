...
import { Sync, SyncAlt } from "@material-ui/icons";
...
const AddQuestions = (props) => {
  ...
  const [surveyId, setSurveyId] = useState(props.match.params.survey_id);
  ...
  const setDetails = async () => {
    if (questionsList.questions && questionsList.name) {
      console.log("questionsList",questionsList);

      await setSurveyName(questionsList.name);
      console.log("surveyName",surveyName);
      let questionArray = questionsList.questions;
      setListQuestion(questionArray);
      if (props.match.params.question_id) {
        console.log("QQQ", props.match.params.question_id);
        let questionId = props.match.params.question_id;
        let singleData = questionArray.find((i) => {return i.id == questionId});
        console.log("Single data", singleData);
        setData(singleData);
      }
      if (props.match.params.type=='add') {
        dispatch(setToolbarHeader(`Add new question (${questionsList.name})`));
      } else {
        dispatch(setToolbarHeader(`Update question (${questionsList.name})`));
      }
    }
  ...
  useEffect(() => {
    if (!questionsList.name) {
      dispatch(fetchQuestionList(surveyId)).then(() => {
        setTimeout(() => {
        setDetails();
        }, 1000);
      })
  ...
  useEffect(() => {
    setDetails();
    setOrder(questionsList.questions?questionsList.questions.length+1:1);
  }, [ questionsList,questionTypes ]);
          ...
          ? options && options.map((val, index) => {
              return (
                <>
                  ...
                  <Grid key={ index } item xs={ 12 } sm={ 2 } md={ 5 } lg={ 5 }>
                  <FormControl fullWidth>
                    <InputLabel id="demo-simple-select-label">Next Question</InputLabel>
                    <Select
                      labelId="demo-simple-select-label"
                      id="demo-simple-select"
                      value={ val.next_question_id }
                      ...
                      onChange={(event) => {
                        let newdata = [ ...options ];
                        let temp_element = { ...newdata[index] };
                        temp_element.next_question_id = event.target.value;
                        newdata[index] = temp_element;
                        setOptions(newdata);
                      }}
                      >
                      <MenuItem key={`q`} value="">Auto</MenuItem>
                      { listQuestion && listQuestion.map((item,index) => {
                        return <MenuItem key={`q${index}`} value={ item.id }>({ item.order}) { item.title}</MenuItem>
                      })}
                  ...
                  <Grid key={index} item xs={2} sm={2} md={1} lg={1}>
                    <TextField
                      ...
                      required
                      inputProps={{ maxLength: 50 }}
                      value={val.order?val.order:options.length}
                      onChange={ (e) => {
                        let newdata = [ ...options ];
                        let temp_element = { ...newdata[index] };
